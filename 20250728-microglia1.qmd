...
The reclustering with resolution = 0.3 identified seven transcriptionally distinct microglial subpopulations (clusters 0–6), compared with the initial single microglia cluster in the whole-brain dataset. These clusters exhibited clear separation in UMAP space, indicating distinct transcriptional programs.
...
colnames(microglia@meta.data)[idx] <- paste0(sig_names, "_Score")
```
...
signature_plot <- FeaturePlot(microglia, features = c("DAM_Score" ,"IRM_Score","IEG_Score",
                                                      "Trem2", "Ifi207", "Nfkb1", 
                                                      "P2ry12","Ccr5","Il1a",
                                                      "Cxcl10","Cd74","H2-D1"),
...
wrap_plots(signature_plot, ncol = 3)
```
...
rownames(data4cluster) = paste("cluster",0:6)


    
 ph <- pheatmap(data4cluster,
          cellwidth = 10, cellheight = 10,
          angle_col = 45, row.names=T) 
 

gene_sets <- cutree(ph$tree_col, k = 7) |> 
    as.data.frame() |> 
    set_names("gene_set") |>
    rownames_to_column("gene_marker")


```

### Gene sets (clusters) in the hierarchical cluster

<details>

<summary>**Discriminant sets & Functions**</summary>

__Set 1__: _early-activated, NF-κB-driven, pro-inflammatory microglia_

 

```{r}
cat(gene_sets$gene_marker[gene_sets$gene_set==1], sep = ",")
```

**Immediate-early response and transcriptional control**
Genes like *Egr1*, *Atf3*, *Btg2*, *Ier2*, and *Zfp36* are classic immediate-early genes (IEGs). They respond rapidly to stress or cytokine signals and reprogram transcription. Many are regulated downstream of NF-κB or AP-1 and mark an “alerted” or reactive microglial state.

**NF-κB signaling and feedback loops**
*Nfkbia* and *Nfkbiz* are NF-κB inhibitors (IκB family). *Tnfaip3* (A20) is a ubiquitin-editing enzyme that dampens NF-κB and TNF receptor signaling. Together they form negative-feedback regulators preventing runaway inflammation.

**Cytokine and pro-inflammatory signaling**
*Il1a*, *Il1b*, and *Tnf* are canonical pro-inflammatory cytokines central to microglial reactivity. They drive neuroinflammation and are elevated near amyloid plaques. *Cd83* also participates in cytokine-mediated immune activation.

**Post-transcriptional and mRNA turnover**
*Zfp36* encodes tristetraprolin, which binds AU-rich elements and destabilizes TNF, IL-1, and other pro-inflammatory mRNAs. This provides post-transcriptional brakes on cytokine expression.

**Immune cell activation and migration**
*Plek* (pleckstrin) links to cytoskeletal remodeling, adhesion, and vesicle trafficking in leukocytes, contributing to microglial mobility and phagocytic activity.
...
```{r}
cat(gene_sets$gene_marker[gene_sets$gene_set==4], sep = ",")
```


**1. Antigen processing and presentation (MHC-II pathway)**

* *H2-Aa*, *H2-Eb1*, *H2-Ab1*: encode MHC class II subunits, enabling presentation of antigens to CD4⁺ T cells.
* This reflects **immune–adaptive interface**, increasingly observed in AD microglia, especially in plaque-associated niches.

**2. Phagolysosomal and vesicular pathways**

* *Atp6v0d2*: subunit of vacuolar H⁺-ATPase, essential for acidification of lysosomes, autophagy, and degradation of engulfed material.
* Supports the DAM-linked lysosomal upregulation program.

**3. Microglial surface receptors and adhesion**

* *Itgax* (CD11c): integrin alpha X, a marker of activated microglia/DAM, enhances adhesion and phagocytosis.
* Also used experimentally to define “CD11c⁺ microglia” in neurodegeneration.

**4. Extracellular remodeling, repair, and trophic factors**

* *Spp1* (osteopontin): secreted glycoprotein involved in cell adhesion, survival, and inflammatory signaling. In AD, strongly upregulated in plaque-associated microglia.
* *Gpnmb*: glycoprotein involved in tissue remodeling, lysosomal function, and protective/anti-inflammatory responses. Often co-expressed with *Spp1* in DAM microglia.

**5. Lipid handling and metabolic reprogramming**

* *Fabp5*: fatty acid binding protein, mediates lipid uptake, trafficking, and metabolic rewiring during inflammation.
* Lipid metabolism shifts are central to DAM and AD progression.

---

__Set 5__: _interferon-responsive microglia (IRM)_

```{r}
cat(gene_sets$gene_marker[gene_sets$gene_set==5], sep = ",")
```


**Interferon-stimulated genes (ISGs)**

* *Ifi207, Ifi211, Ifi206, Ifi213*: murine interferon-inducible family members.
* *Ifit2, Ifit3, Ifit3b*: interferon-induced proteins with tetratricopeptide repeats (antiviral defense, translation inhibition).
* *Isg15*: ubiquitin-like modifier that conjugates to proteins (“ISGylation”), modulating innate immunity.
* *Rsad2* (viperin): antiviral effector blocking viral replication and regulating lipid metabolism.
* *Oasl2*: 2′–5′-oligoadenylate synthetase-like, amplifies RIG-I signaling.
* *Mx1*: dynamin-like GTPase that restricts viral transcription/replication.

---

__Set 6__: _down-regulation of complement-activating microglia_

```{r}
cat(gene_sets$gene_marker[gene_sets$gene_set==6], sep = ",")
```

Cluster 5 in the microglia. Downregulation of C1qa/b/c indicates that microglia are not actively engaging the complement–synapse pruning program. Instead, they may represent a later or alternative activation state, possibly evolving towards interferon-responsive microglia (IRM).

---

**Klf2** & **Ccl4** _Regulators of NF-κB_
 
**Klf2** encodes a zinc-finger transcription factor regulating cell cycle, survival, and stress responses. *Klf2* is a **transcriptional regulator that restrains microglial activation**, acting as a **negative feedback factor** on NF-κB and cytokine-driven responses. In Alzheimer’s disease, its expression status may mark whether microglia are keeping an anti-inflammatory brake (*Klf2* high) or shifting toward a pro-inflammatory, DAM/IRM-like state (*Klf2* low). Some transcriptomic studies in neurodegeneration show *Klf2* as part of a “brake” module, countering the pro-inflammatory DAM and IRM signatures.


**Ccl4** (macrophage inflammatory protein-1β, MIP-1β) encodes a CC chemokine that binds CCR5 and CCR8. Recruits monocytes, T cells, NK cells to inflamed tissue.Its expression is rapidly induced by NF-κB, TNF, and interferons.*Ccl4* is also part of the interferon response, acting in antiviral defense and immunomodulation.It strengthens the NF-κB-driven cytokine module and mediates recruitment of peripheral immune cells via CCR5. Functionally, it bridges resident microglial activation and broader neuroinflammation, positioning it at the **intersection of DAM and interferon-responsive microglial states**.
